 ...
 *
 * `/help` is generated from this array, so a new command only needs to be added
 * here (spec + handler) to show up everywhere. The handler receives a
 * {@link CommandApi} supplied by the REPL, which is how commands drive the UI
 * (switch screens, change model, print output, etc.) without importing React.
 */
export interface CommandApi {
  /** Print a line into the transcript (system/info output). */
  print(text: string): void;
  /** Show the dynamically-generated help text. */
  showHelp(): void;
  /** Open the settings screen. */
  openSettings(): void;
  /** Clear the saved API key and exit to the login screen next launch. */
  logout(): void;
  /** Open the model picker. */
  openModelPicker(): void;
  /** Scaffold a new project in the current directory. */
  initProject(args: string): void;
  /** Change the working directory. */
  changeFolder(args: string): void;
  /** Spin up a named subagent. */
  runAgent(args: string): void;
  /** Show past sessions / resume. */
...
export interface CommandSpec {
  ...
  summary: string;
  usage?: string;
  handler: (api: CommandApi, args: string) => void;
}
...
export function parseCommand(input: string): { spec?: CommandSpec; args: string } | null {
...
export function formatHelp(): string {
  const width = Math.max(
    ...commands.map((c) => `/${c.name}${c.aliases ? ` (/${c.aliases.join(', /')})` : ''}`.length),
  );
